...
app.post("/penyimpanan", (req, res) => {
  ...
  // Cari data pada tanggal sebelumnya dengan lokasi yang sama
  db.query(
        ...
        db.query("SELECT * FROM data_pkm WHERE id_penyimpanan = ?", [previousId], (err, pkmResults) => {
          ...
          db.query("SELECT * FROM kernel WHERE id_penyimpanan = ?", [previousId], (err, kernelResults) => {
            ...
            if (kernelResults.length > 0) {
              ...
              totalKernel.kadar_kotoran += Number(kernelResults[0].kadar_kotoran);
            ...
            db.query("SELECT * FROM kategori WHERE id_penyimpanan = ?", [previousId], (err, kategoriResults) => {
              ...
              kategoriResults.forEach(katPrev => {
                ...
                if (matchingKategori) {
                  // Ambil jumlah total sebelumnya
                  db.query("SELECT * FROM jumlah_total WHERE id_kategori = ?", [katPrev.id], (err, jumlahResults) => {
                    if (err) return res.status(500).json(err);
                    if (jumlahResults.length > 0) {
                      matchingKategori.jumlah.stok += Number(jumlahResults[0].stok);
                      matchingKategori.jumlah.alb += Number(jumlahResults[0].alb);
                      matchingKategori.jumlah.kadar_air += Number(jumlahResults[0].kadar_air);
                      matchingKategori.jumlah.kadar_kotoran += Number(jumlahResults[0].kadar_kotoran);
                      matchingKategori.jumlah.do += Number(jumlahResults[0].do);
                      matchingKategori.jumlah.hi += Number(jumlahResults[0].hi);
                    }

                    // Ambil penyimpanan sebelumnya
                    db.query("SELECT * FROM penyimpanan WHERE id_kategori = ?", [katPrev.id], (err, penyimpananResults) => {
                      if (err) return res.status(500).json(err);

                      penyimpananResults.forEach(pPrev => {
                        const matchingPenyimpanan = matchingKategori.penyimpanan.find(p => p.jenis_tank === pPrev.jenis_tank);
                        if (matchingPenyimpanan) {
                          matchingPenyimpanan.stok += Number(pPrev.stok);
                          matchingPenyimpanan.alb += Number(pPrev.alb);
                          matchingPenyimpanan.kadar_air += Number(pPrev.kadar_air);
                          matchingPenyimpanan.kadar_kotoran += Number(pPrev.kadar_kotoran);
