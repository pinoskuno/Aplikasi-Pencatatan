            ...
            <Table striped bordered hover>
              ...
              <tbody>
                {filteredDataPrevious.map((item, index) => (
                  <>
                    {/* Baris utama untuk tanggal dan lokasi */}
                    <tr key={`main-${index}`}>
                      <td
                        rowSpan={Object.keys(item.kategori).reduce(
                          (acc, key) =>
                            acc + item.kategori[key].penyimpanan.length + 3, // Tidak menghitung PKM dan Kernel di rowspan
                          1
                        )}
                      >
                        {new Date(item.tanggal).toLocaleDateString()}
                      ...
                      <td
                        rowSpan={Object.keys(item.kategori).reduce(
                          (acc, key) =>
                            acc + item.kategori[key].penyimpanan.length + 3,
                          1
                        )}
                      >
                        {item.lokasi}
